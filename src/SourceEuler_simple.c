     ...
     real dt;
{
  ...
  int i, j, l, nr, ns, k;
  int lip, li2p;
  real r, rip, ri2p, qpip, qpi2p, ff, tau, taua;
...
#pragma omp for
	...
	if (viscosity != 0.0) {
	  ...
	  qplus[l] = 0.5*frac*(taua/tau)/viscosity/dens[l]*(Trr[l]*Trr[l] +	\
					    Trp[l]*Trp[l] +	\
					    Tpp[l]*Tpp[l] );
	...
	if (Cooling) {
	 ...
	 //CoolInit(Energy, Rho);
          //num = dt*qplus[l] + energy[l];
	  if (energy[l] > 0.0 || 1) printf("%1.12f\t%1.12f\t%1.12f\t qcoola=%1.12f\t dt=%1.12f\n",dt*qplus[l],energy[l],dt*qcoola[l],qcoola[l],dt);
   ...
   /* if (RADIALCOOL){
    ...
    for (i = 0; i < nr; i++) {
	  ...
	  l = j+i*ns;
        ...
        tccgs = temp[l]*TEUNIT;
        //opa[l]=opac(max2(1.0e-4/2./hei,rhoccgs), max2(TENV*TEUNIT,tccgs));
        opa[l]=10.0;

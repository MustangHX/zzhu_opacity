     ...
     real step;
{
  ...
  DRMIN = GlobalRmed[GLOBALNRAD-1]*0.1;
  DRMAX = GlobalRmed[GLOBALNRAD-1]*0.85;
  ...
  for (i = Zero_or_active; i < Max_or_active; i++) {
    if ( (Rmed[i] < DRMIN) || (Rmed[i] > DRMAX) ) {
      ...
      if (Rmed[i] < DRMIN) {
	damping = 0.0+1.0*pow((Rmed[i]-DRMIN)/(GlobalRmed[0]-DRMIN),0.3);
	
	//damping = 1.0;
	lambda = damping*damping*1000.0*step/Tin;
      }
      if (Rmed[i] > DRMAX) {
	
	damping = 0.0+1.0*pow((Rmed[i]-DRMAX)/(GlobalRmed[GLOBALNRAD-1]-DRMAX),0.3);
	//damping = 1.0;
	lambda = damping*damping*1000.0*step/Tout;
      }
...
//      if (!SelfGravity) {
  ...
  //    if (SelfGravity) {
    ...
    if (ViscosityAlpha) {
        vrad0 = -3.0*viscosity/Rmed[i]*(-SIGMASLOPE+2.0*FLARINGINDEX+1.0);
      } else {
	...
	vtheta0=VthetaMed[i];	
      dens0 = SigmaMed[i];
      energ0 = EnergyMed[i]*0.41735736782;
      //if(i==0) printf("EEEEE0::::%2.12g\t",energ0*(ADIABATICINDEX-1.0)/SigmaMed[i]*LUNIT*LUNIT/TUNIT/TUNIT/8.31446e7);
//	if (energ0 <=0.0 || dens0 <=0.0) printf("EEEEE%e\t%e\n",energ0,dens0);
      for (j = 0; j < ns; j++) {
	l = i*ns + j;
	...
	//printf("%1.12f\t%1.12f\n",vrad0,lambda);
//	if(l==0) printf("EEEEE0::::%2.12g\t",energ[l]*(ADIABATICINDEX-1.0)/SigmaMed[i]*LUNIT*LUNIT/TUNIT/TUNIT/8.31446e7);
		}
      }
    }
  }

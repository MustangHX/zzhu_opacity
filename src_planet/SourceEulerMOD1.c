...
for (i=0;i<MAX1D;i++){
	if (CPU_Rank==0) {
		...
		for(j=0;j<CPU_Highest;j++){
                	MPI_Send (&value,1, MPI_DOUBLE, j+1, 13, MPI_COMM_WORLD);
		}
	}
	if (CPU_Rank>0) {
                MPI_Recv (&value, 1, MPI_DOUBLE, 0, 13, MPI_COMM_WORLD, &stat);
	}
	OpacMed[i]=value;
}
}*/
...
//printf("%f\t%1.20f\n",Rmed[i],qplus[l]);
	...
	if (Cooling) {
	 ...
	 //CoolInit(Energy, Rho);
          //num = dt*qplus[l] + energy[l];
	  //if ((energy[l] < 0.0 || 0) && i==10 || j==0) printf("%g\t%g\t%g\t qplus=%g\t qcoola=%g\t %g\t dt=%g\t %d\t%d\n",dt*qplus[l],energy[l],dt*qcoola[l],qplus[l],qcoola[l],Trp[l],dt,i,j);
     ...
     PolarGrid *Energy,*Rho;
{
    ...
    real hei, heia, rhoc, rhoccgs, tccgs, kappacgs, tau, taua, Text4, Lstarcgs, Lacccgs, alpha, dens_temp;
   ...
   /* if (RADIALCOOL){
    ...
    for (i = 0; i < nr; i++) {
      for (j = 0; j < ns; j++) {
	  l = j+i*ns;
	//if (energ[l] <=0.0) printf("ENERGY: %g\t %d\t %d\n",energ[l],i,j);
        if (Restart && 1) {
	energ[l]=EnergyMed[i];
	}
	...
	if (Restart && 0) {
        temp[l] = MU/R*(ADIABATICINDEX-1.0)*EnergyMed[i]/rho[l];
	}
	else temp[l] = MU/R*(ADIABATICINDEX-1.0)*energ[l]/rho[l];
        //tccgs = temp[l]*TEUNIT;
        tccgs = temp[l]*LUNIT*LUNIT/TUNIT/TUNIT/8.31446e7;
//	opa[l]=opac(max2(1.0e-4/2./hei,rhoccgs), (tccgs));
 	opa[l]=opac((rhoccgs), (tccgs),i,j,hei,rho[l],energ[l],temp[l]); 
	//opa[l]=OpacMed[i];
      //opa[l]=10.0;

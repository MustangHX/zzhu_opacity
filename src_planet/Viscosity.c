...
real FViscosity (rad)
     real rad;
{
  real viscosity, rmin, rmax, scale,dr_tran,r_tran;
...
//  rmax *= scale;
  ...
  rmin *= scale;
  rmax *= scale;
  dr_tran = DRTRAN*scale;
  r_tran =  RTRAN*scale;

  //if (rad < rmin) viscosity *= CAVITYRATIO;
if (rad < rmin) viscosity *=  VISCOSITYRATIO; 
 if ((rad >= rmin) && (rad <= rmax)) {
    //viscosity *= exp((rmax-rad)/(rmax-rmin)*log(CAVITYRATIO));
    viscosity *= 0.5*(1.0-VISCOSITYRATIO)*sin((rad-r_tran)*M_PI/(2.0*dr_tran))+0.5*(1.0+VISCOSITYRATIO);
